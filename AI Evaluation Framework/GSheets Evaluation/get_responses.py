...
gateway csv_questions.py uses). Three things make this more than a copy of
csv_questions.py:
  ...
  * Reasoning mode — models whose RESULTS TAB name contains the word "Reasoning" (e.g.
    "Claude Opus 4.6 (Reasoning)") are called with the gateway's reasoning turned up to
    its highest setting (reasoning_effort="high"); every other model is called with no
    reasoning parameter at all, i.e. the gateway default. The same base model can
    therefore be evaluated both ways just by pointing it at a plain tab vs. a
    "(Reasoning)" tab.

Exposes get_responses_df(prompts_df, model, model_tab_name="") -> DataFrame with columns
'Prompt ID' and 'Model Output Response'. Reasoning is decided from model_tab_name. Run
standalone (reads GOOGLE_SHEET_TAB + MODEL + MODEL_OUTPUT_TAB from .env) for a quick check,
or import it into the evaluate_model.py driver.
"""
...
def get_responses_df(prompts_df, model, model_tab_name=""):
    ...
    If `model_tab_name` contains the word "Reasoning" (case-insensitive), every call is
    made with reasoning_effort="high"; otherwise no reasoning parameter is sent and the
    gateway uses its default. The flag is derived once from the tab name because a run
    evaluates a single model into a single tab.
    """
...
def main():
    ...
    # MODEL_OUTPUT_TAB drives the reasoning decision, matching what the driver passes.
