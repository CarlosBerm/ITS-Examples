...
Where evaluate_model.py evaluates a single model, this driver reads a Model Directory tab
(Model ID + Model Tab Name) from the SAME Google Sheet and runs the pipeline once per model,
writing each model's responses into its own results tab.
...
Pipeline:
    ...
    4. per model: get responses (per-prompt errors are recorded AS the response text, same
       as evaluate_model.py) and write the tab (get_responses.get_responses_df,
...
The spreadsheet comes from GOOGLE_SHEET_ID in .env (all tabs live in the same spreadsheet).
Writing results requires the sheet shared with the service account as an EDITOR.
"""
...
def main():
    ...
    print(f"  {len(directory_df)} model(s) with both a Model ID and a Database Tab.\n")
    ...
    print("Checking that each Model Tab Name exists in the spreadsheet...")
    ...
    for _, row in directory_df.iterrows():
        ...
        if tab in titles:
            ...
            print(
                ...
                f"spreadsheet — skipping. Available tabs: {available}"
            )
            results.append((model_id, tab, "skipped", "Model Tab Name not found"))

    if not valid:
        print("\nNo models with a locatable Database Tab — nothing to evaluate.\n")
    ...
    #    unexpected failure of the whole stage (e.g. a bad key or a write/permission error)
    #    skips that one model, so a single infra hiccup doesn't kill the whole batch. -----
    ...
    for i, (model_id, tab) in enumerate(valid, start=1):
        ...
        try:
            ...
            # A pipeline stage called sys.exit (e.g. a write/permission error). Its own
