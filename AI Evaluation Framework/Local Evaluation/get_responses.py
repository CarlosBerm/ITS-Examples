"""
get_responses.py — send every prompt in the prompt library to the model and record each
response.

This is the shared "ask the model" stage: it is intentionally identical to
GSheets Evaluation/get_responses.py. Only where the prompts come from (a local workbook here,
a Google Sheet there) and where the responses go differs between the two modes — the model
...
make this more than a simple loop:
  ...
  * Context retention — every turn shares ONE running conversation, so prompts that probe
    memory ("...ten prompts later, what is my student ID?") have the earlier turns to draw
    on. Storing the whole history is what gives the model that memory, but on its own it made
    the model sometimes replay and re-answer every earlier prompt. A guardrail added to the
    system message fixes that: treat the history as memory, but answer only the newest turn.

  * Error handling — some prompts deliberately test prompt injection / policy-breaking. Those
    can make the gateway reject the request (a raised exception, distinct from the model
...
# Assistant persona + current U-M facts injected as context so models with older training
# cutoffs still answer from the current state. These facts are TIME-SENSITIVE (verified July
# 2026) — refresh them before a formal eval run, since a stale value here would effectively be
# graded as ground truth (e.g. PRMPT-12's president probe).
SYSTEM_PROMPT = (
...
# Added guardrail (this is the "extra context" that stops the model from replaying the whole
# conversation). It does NOT change the persona above and deliberately adds no anti-injection
# coaching, so the safety/robustness prompts stay a fair test.
...
def get_responses_df(prompts_df, model, model_tab_name=""):
    ...
    for index, row in prompts_df.iterrows():
        ...
        try:
            ...
            # Injection / policy-breaking prompts can make the gateway reject the call. Record
            # it and move on; append a neutral stub so the conversation stays a well-formed
            # user/assistant alternation for the turns that follow.
